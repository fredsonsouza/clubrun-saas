...
export function MapView({ routeData }: MapViewProps) {
  ...
  const MAP_STYLES = [
    { id: 'mapbox://styles/mapbox/outdoors-v12', name: 'Outdoor', icon: <Layers className="h-4 w-4" /> },
    { id: 'mapbox://styles/mapbox/satellite-streets-v12', name: 'Satélite', icon: <Globe className="h-4 w-4" /> },
    { id: 'mapbox://styles/mapbox/light-v11', name: 'Light', icon: <Layers className="h-4 w-4 opacity-50" /> },
    { id: 'mapbox://styles/mapbox/dark-v11', name: 'Dark', icon: <Layers className="h-4 w-4 text-gray-400" /> },
  ]
    ...
    <div className="relative h-[650px] w-full overflow-hidden rounded-[3rem] border border-gray-100 shadow-2xl">
      ...
      <div className="absolute top-8 left-8 z-10 flex flex-col gap-4">
        <div className="rounded-3xl bg-white/90 p-6 shadow-xl backdrop-blur-md">
          <h4 className="text-xs font-black uppercase tracking-widest text-orange-500">Percurso Oficial</h4>
          <p className="text-sm font-bold text-gray-900">Explore o trajeto da prova</p>
        </div>
          ...
          <div className="flex gap-1">
            {MAP_STYLES.map((style) => (
              <button
                ...
                className={`flex h-10 items-center gap-2 rounded-2xl px-4 text-[10px] font-black uppercase tracking-tighter transition-all ${
                  mapStyle === style.id
      ...
      <div className="absolute bottom-8 left-8 z-10 hidden rounded-2xl bg-black/40 px-4 py-2 text-[10px] font-bold text-white backdrop-blur-sm sm:block">
