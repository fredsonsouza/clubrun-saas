  ...
  return (
    <div className="min-h-screen bg-[#0f172a] font-sans text-slate-200 selection:bg-orange-500/30 overflow-hidden">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div
          className={`absolute top-[-10%] left-[-10%] w-[40%] h-[40%] rounded-full blur-[120px] opacity-20 bg-gradient-to-br ${slides[currentSlide].color} transition-all duration-1000`}
        />
        <div
          className={`absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] rounded-full blur-[120px] opacity-10 bg-gradient-to-br ${slides[currentSlide].color} transition-all duration-1000`}
        />
      </div>
      ...
      <main className="relative z-10 flex flex-col lg:flex-row items-center justify-center min-h-[calc(100vh-160px)] px-6 md:px-20 gap-12 lg:gap-20">
        ...
        <div className="w-full lg:w-1/2 flex flex-col items-start space-y-8 animate-in fade-in slide-in-from-left-8 duration-700">
          <div className="space-y-4">
            <div
              className={`inline-flex items-center gap-2 px-4 py-2 rounded-2xl bg-white/5 border border-white/10 text-sm font-bold tracking-widest uppercase transition-colors duration-1000 text-orange-400`}
            >
            ...
            <h2
              className={`text-2xl md:text-3xl font-bold bg-gradient-to-r ${slides[currentSlide].color} bg-clip-text text-transparent transition-all duration-1000`}
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
            {slides[currentSlide].features.map((feature, i) => (
              <div
                key={i}
                className="flex items-center gap-3 p-4 rounded-2xl bg-white/5 border border-white/5 hover:border-white/10 transition-colors group"
              >
                <div
                  className={`h-8 w-8 shrink-0 flex items-center justify-center rounded-lg bg-gradient-to-br ${slides[currentSlide].color} opacity-80 group-hover:opacity-100 transition-opacity`}
                >
        ...
        <div className="w-full lg:w-1/2 flex items-center justify-center animate-in fade-in zoom-in-95 duration-1000 delay-200">
          <div className="relative w-full max-w-lg aspect-square">
            ...
            <div
              className={`absolute inset-0 bg-gradient-to-br ${slides[currentSlide].color} rounded-[3rem] opacity-20 blur-[60px] animate-pulse`}
            />
            ...
            <div className="relative h-full w-full rounded-[3rem] border border-white/10 bg-slate-900/50 backdrop-blur-xl p-10 flex flex-col items-center justify-center text-center space-y-8 shadow-2xl overflow-hidden group">
              ...
              <div
                className={`relative h-32 w-32 rounded-3xl bg-gradient-to-br ${slides[currentSlide].color} flex items-center justify-center shadow-2xl transform transition-transform group-hover:scale-110 group-hover:rotate-3 duration-500`}
              >
                {React.cloneElement(
            ...
            {/* Floating elements */}
            <div
              className={`absolute -top-4 -right-4 h-20 w-20 rounded-2xl bg-slate-800 border border-white/10 flex items-center justify-center shadow-2xl animate-bounce duration-[3000ms]`}
            >
              <Activity className="h-10 w-10 text-orange-500" />
            </div>
            <div
              className={`absolute -bottom-6 -left-6 h-24 w-24 rounded-3xl bg-slate-800 border border-white/10 flex items-center justify-center shadow-2xl animate-pulse`}
            >
