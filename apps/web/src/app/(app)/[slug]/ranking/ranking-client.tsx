            ...
            <div className="flex rounded-2xl border border-gray-100 bg-white p-1.5 shadow-sm">
              {(['week', 'month', 'year'] as const).map((p) => (
                <button
                  key={p}
                  onClick={() => setPeriod(p)}
                  className={`cursor-pointer rounded-xl px-5 py-2 text-xs font-black tracking-wider uppercase transition-all ${period === p ? 'bg-orange-500 text-white shadow-md shadow-orange-500/20' : 'text-gray-400 hover:text-gray-900'}`}
                >
