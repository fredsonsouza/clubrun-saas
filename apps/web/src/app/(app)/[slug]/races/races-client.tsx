...
export function RacesClient({
  ...
  const handleToggleRegistration = async (raceId: string) => {
  ...
  const handleDeleteRace = async () => {
    ...
    try {
      await deleteRace(club.slug, selectedRace.id)
                    ...
                    <div className="flex items-center gap-2">
                      {canManage && (
                        <div className="flex gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                          <button 
                            onClick={(e) => { e.preventDefault(); toast.info('Funcionalidade de edição em breve.') }}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {activeTab === 'upcoming' ? (
                        <button
                          onClick={(e) => {
                            e.preventDefault()
                            handleToggleRegistration(race.id)
                          }}
                          disabled={isToggling === race.id}
